            ...
            <a href="{{ route('sneakers.show', $sneaker->id) }}" class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100 text-center">
                ...
                <div class="flex flex-row justify-between mt-4 sm:flex hidden">
                    <div class="flex justify-center items-center lg:flex md:hidden">
                        <form method="POST" action="{{ route('sneakers.like', $sneaker->id) }}">
                            ...
                            <button type="submit" class="text-white py-2 rounded-md">
                                @if ($sneaker->likedByUsers->contains(auth()->user()->id))
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:text-amber-200">
                        ...
                        <span class="text-xl font-semibold mx-4 text-red-500">{{ $sneaker->dislikes }}</span>
                        <form method="POST" action="{{ route('sneakers.dislike', $sneaker->id) }}">
                            @csrf
                            <button type="submit" class="text-white py-2 rounded-md">
                                @if ($sneaker->dislikedByUsers->contains(auth()->user()->id))
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:text-red-500">
